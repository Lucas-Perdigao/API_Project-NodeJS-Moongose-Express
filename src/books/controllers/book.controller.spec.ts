...
import { fakeBookData, fakeId, validFakeAuthorParam } from "../__mocks__/fake.book.data"
...
describe("BookController", () => {  
    describe("getAll", () => {
        ...
        it("should return a list of books by author", async () => {
            req.query.author = validFakeAuthorParam
            await bookController.getAll(req, res)
    ...
    describe("updateStatus", () => {
        ...
        it("should return a promiseError", async () => {
            req.params.id = fakeId
            req.body = fakeBookData[0]
            jest.spyOn(fakeBookService, "updateStatus").mockImplementation(() => Promise.resolve(promiseError("error")))
            await bookController.updateStatus(req, res)
            expect(res.status).toHaveBeenCalledWith(StatusCode.OK)
        })

        it("should return a invalidIdError", async () => {
            jest.spyOn(fakeBookService, "updateStatus").mockImplementation(() => Promise.resolve(invalidIdError("id")))
            await bookController.updateStatus(req, res)
            expect (res.status).toHaveBeenCalledWith(StatusCode.BAD_REQUEST)
        })
    })
